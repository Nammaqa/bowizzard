...
import { Sidebar, SidebarFooter, SidebarGroup, SidebarGroupContent, SidebarGroupLabel, SidebarHeader, SidebarMenu, SidebarMenuButton, SidebarMenuItem, SidebarProvider } from "./components/ui/sidebar";
              ...
              <SidebarMenuItem key={'Dashboard'}>
                <SidebarMenuButton asChild className="p-5 flex items-center">
                  <a href={'/dashboard'}>
                    ...
                    <span className='ml-4' style={{ fontSize: '14px' }}>Dashboard</span>
                  </a>
                </SidebarMenuButton>
              </SidebarMenuItem>
            </SidebarMenu>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {careerMap.map((item, idx) => (
                <SidebarMenuButton asChild className="p-5 flex items-center" key={item.label + idx}>
                  <a href={item.href}>
                    {item.icon}
                    <span className='ml-4' style={{ fontSize: '14px' }}>{item.label}</span>
                  </a>
                </SidebarMenuButton>
              ))}
            </SidebarMenu>
        ...
        <SidebarFooter className='mt-auto mb-4'>
          ...
          >
            <a href={'logout'}>
              ...
              <span className='ml-4' style={{ fontSize: '14px', color: "#FF0000" }}>Logout</span>
            </a>
          </SidebarMenuButton>
        </SidebarFooter>
      </Sidebar>
      <main className='bg-[#F0F0F0] min-h-screen flex-1'>
        {children}
      </main>
    </SidebarProvider>
  );
